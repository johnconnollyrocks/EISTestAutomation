...
/**
 * Test class - Test_IPP_REST_Create_Shopper 
 * 
 * @author Santosh
 * @version 1.0.0
 */
public final class Test_IPP_REST_Negative_Update_Expiration_Year extends BornInCloudTestBase {
	...
	String expireYear="1989";
	
	
	
	public Test_IPP_REST_Negative_Update_Expiration_Year() throws IOException {
	...
	@Test
	public void Test_IPP_REST_Negative_Update_Expiration_Year_Method() throws Exception {
		...
		// Parsing test data values from test data file and creating a test data Hash
				...
				HttpResponse response = updateEntitlement(client, testDataMap.get("userId"),testDataMap.get("ippRequest"),actualRequestJson, testDataMap.get("jsonMimetype"));	
				
				
							    
				// Verifying the Status Code 
				System.out.println("Assert : Verify the Status Code" + "\n");
				compareStrings(response.getStatusLine().getStatusCode(), HttpStatus.SC_BAD_REQUEST);
			    ...
			    compareStrings(expectedResponceJson.toUpperCase().replace(" ", ""), actualjsondata.toUpperCase().replace(" ", ""));  
	...
	public String getStringAfterReplacingData(String string){
		...
		temp2=temp1.replace("CARD_EXPMONTH", expireMonth);
		temp1=temp2.replace("CARD_EXPYEAR", expireYear);
