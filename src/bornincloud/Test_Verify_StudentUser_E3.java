...
public class Test_Verify_StudentUser_E3 extends BornInCloudTestBase {

	public Test_Verify_StudentUser_E3() {
		super("browser");
	...
	public void Test_Verify_StudendUser() throws Exception {
			...
			for (int i = 0; i < serviceslist.size(); i++) {
				...
				if (name.contains("Fusion 360")) {
					assertTrueCatchException(
							"Fusion 360 Student is Displayed in Services",
		...
		if (!service_fusion) {
			assertTrueCatchException("Fusion 360 Should be Displayed in Services",
					false);
		...
		if (homePage.checkFieldExistence("productslist")) {
			...
			for (int i = 0; i < productslist.size(); i++) {
				...
				if (name.contains("Fusion 360 Student")) {
					assertTrueCatchException(
							"Fusion 360 Should Not be Displayed in Product", false);
					product_fusion = true;
					break;
				}
			}
		...
		if (!product_fusion) {
			assertTrueCatchException("Fusion 360 is not Displayed in Product for Student in E3",
					true);
			Util.printError("Fusion 360 is not Displayed in Product");
		}else{
			assertTrueCatchException("Fusion 360 is not Displayed in Product for Student in E3",
					false);
		}
