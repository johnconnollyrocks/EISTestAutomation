...
public class Test_Verify_VersionPermissionsofCM_EndUser extends CustomerPortalTestBase {
	public Test_Verify_VersionPermissionsofCM_EndUser() throws IOException {
...
//		super();
	...
	public void Test_Verify_OnlineMap() throws Exception {
		...
		mainWindow.select();
		//Setting permissions to the EndUser/CM
		givingVersionPermissionsToUser();
		
		homePage.refresh();
		Util.sleep(4000);
		if(!homePage.isFieldVisible("RestrictedPermissionsOfCM")){
			homePage.verifyFieldNotExists("RestrictedPermissionsOfCM");			
		}else{
			EISTestBase.fail("Version Permissions for CM are not reflected");
		}
	...
	public void givingVersionPermissionsToUser(){
		...
		if(homePage.isFieldVisible("ManageAccessCancelDailogPopup")){
			...
			homePage.click("ManageAccessCancelPopupSave");
			break;
		 }
	   }
		break;
	...
	public void AssignPermissionsToUser(){
		//clicking on manage access versions
